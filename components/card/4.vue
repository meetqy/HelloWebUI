<template>
  <div
    class="w-full max-w-sm px-4 py-3 mx-auto bg-base-100 rounded-md shadow-md"
  >
    <div class="flex items-center justify-between">
      <span class="text-sm font-light text-base-content">
        {{ $faker.commerce.product() }}
      </span>
      <span
        class="px-3 py-1 text-xs btn btn-primary btn-sm uppercase rounded-full"
      >
    ...
    <div>
      <h1 class="mt-2 text-lg font-semibold text-base-content">
        {{ $faker.commerce.productName() }}
      </h1>
      <p class="mt-2 text-sm text-base-content text-opacity-80">
        {{ $faker.commerce.productDescription() }}
      </p>
    </div>

    <div>
      <div class="flex items-center mt-2 text-base-content text-opacity-70">
        <span>{{ $faker.word.conjunction() }}:</span>
        <a class="btn btn-link">{{ $faker.word.adverb() }}</a>
        <span>{{ $faker.word.noun() }}</span>
        <a class="btn btn-link">{{ $faker.word.interjection() }}</a>
      </div>

      <div class="flex items-center justify-center mt-4">
        <a class="mr-2 btn btn-link">
          <svg
